...
export async function GET() {
  ...
  // Las consultas no dependen entre sí: se lanzan a la vez.
  const [
    [[metricas]], [conteos], [etiquetas], [destacadas], [miembros], [[guardados]],
  ] = await Promise.all([
    pool.query(
    ...
    pool.query("SELECT type, COUNT(*) AS n FROM forum_topics WHERE status='PUBLISHED' GROUP BY type"),
    pool.query(
    ...
    pool.query("SELECT COUNT(*) AS n FROM forum_bookmarks WHERE user_id=?", [me.id]),
  ]);
