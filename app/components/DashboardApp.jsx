...
function AuthScreen({ onAuthenticated }) {
  ...
  return <main className="auth-shell"><section className="auth-visual"><div className="auth-orb auth-orb-one"/><div className="auth-orb auth-orb-two"/><div className="auth-brand"><img src="/branding/logo-grupo-ingenio.png" alt="Grupo Ingenio"/><div><strong>Grupo Ingenio</strong><span>Repositorio documental</span></div></div><div className="auth-kicker">Sistema interno de gestión</div><h1>Repositorio Documental Grupo Ingenio</h1><p>Centraliza procesos, documentos, contratos, evidencias y conocimiento institucional en un entorno seguro para nuestro equipo.</p><div className="auth-capabilities"><span>Documentación controlada</span><span>Procesos institucionales</span><span>Trabajo colaborativo</span></div><div className="auth-security"><ShieldCheck size={17}/><div><b>Acceso exclusivo para colaboradores</b><small>La información se consulta según tu perfil y grupos asignados.</small></div></div></section><section className="auth-card"><div className="auth-card-heading"><span>Bienvenido</span><h2>{mode === "login" ? "Accede al repositorio" : "Crea tu cuenta institucional"}</h2><p>{mode === "login" ? "Ingresa con tus credenciales de Grupo Ingenio." : "Registra tus datos para solicitar acceso al sistema."}</p></div><div className="tabs auth-tabs"><button className={mode === "login" ? "on" : ""} onClick={() => { setMode("login"); setError(""); }}>Iniciar sesión</button><button className={mode === "register" ? "on" : ""} onClick={() => { setMode("register"); setError(""); }}>Crear cuenta</button></div><form className="form-stack" onSubmit={submit}>{mode === "register" && <>{field("full_name", "Nombre completo")}{field("cedula", "Cédula")}</>}{field("email", "Correo electrónico", "email")}{field("password", "Contraseña", "password")}{mode === "register" && field("cargo", "Cargo")}{error && <div className="err">{error}</div>}<button className="btn btn-primary auth-submit" disabled={busy}>{busy ? "Procesando…" : mode === "login" ? "Ingresar al repositorio" : "Registrar y continuar"}</button></form><p className="auth-help">Uso interno · Grupo Ingenio Innovación Organizacional</p></section></main>;
}
