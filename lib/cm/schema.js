...
export async function notificar(pool, { userId, actorId, titulo, mensaje, topicId, tipo = "forum" }) {
  if (!userId || Number(userId) === Number(actorId)) return; // nadie se notifica a sí mismo
  await pool
    .query(
      "INSERT INTO system_alerts (user_id, related_type, related_id, title, message, severity, status) VALUES (?,?,?,?,?, 'info', 'unread')",
      [userId, tipo, topicId ?? null, titulo.slice(0, 220), mensaje ?? null]
    )
    .catch(() => {});
}
...
export async function avisarMenciones(pool, me, texto, topicId, titulo) {
  ...
  for (const nombre of nombres) {
    ...
    if (u && !avisados.has(u.id)) {
      avisados.add(u.id);
      await notificar(pool, { userId: u.id, actorId: me.id, topicId, titulo: `${me.full_name} te mencionó`, mensaje: titulo });
    }
  }
}
