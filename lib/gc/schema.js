...
export async function ensureGcSchema() {
  ...
  // Un mismo explorador sirve a dos espacios: los documentos del contrato y
  // las evidencias generales; el ambito separa carpetas y archivos de cada uno.
  await agregarColumnas(pool, "contract_document_folders", [
    ["scope", "VARCHAR(20) NOT NULL DEFAULT 'documentos'"],
  ]);
  await agregarColumnas(pool, "contract_files", [
    ["scope", "VARCHAR(20) NOT NULL DEFAULT 'documentos'"],
  ]);
  ...
  // --- Reuniones ----------------------------------------------------------
  // Cada reunión funciona como una carpeta fechada a la que se anexan el
  // acta, la asistencia y otros soportes.
  await pool.query(`CREATE TABLE IF NOT EXISTS contract_meetings (
    id INT AUTO_INCREMENT PRIMARY KEY,
    contract_id INT NOT NULL,
    ...
    created_by INT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_reunion_contrato (contract_id),
    INDEX idx_reunion_fecha (meeting_date)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

  await pool.query(`CREATE TABLE IF NOT EXISTS contract_meeting_files (
    id INT AUTO_INCREMENT PRIMARY KEY,
    meeting_id INT NOT NULL,
    contract_id INT NOT NULL,
    kind VARCHAR(20) NOT NULL DEFAULT 'anexo',
    ...
    uploaded_by INT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_reunion_arch (meeting_id),
    INDEX idx_reunion_arch_contrato (contract_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

  // --- Mesas de trabajo por persona ---------------------------------------
  // Una reunión sin user_id es del contrato; con user_id es la mesa de
  // trabajo de esa persona, que carga las suyas de forma independiente.
  await agregarColumnas(pool, "contract_meetings", [
    ["user_id", "INT NULL"],
    ["kind", "VARCHAR(20) NOT NULL DEFAULT 'general'"],
  ]);

  // Cronograma de trabajo: uno por persona y contrato, se reemplaza al subir
  // uno nuevo. Es el punto de partida antes de registrar mesas.
  await pool.query(`CREATE TABLE IF NOT EXISTS contract_workplans (
    id INT AUTO_INCREMENT PRIMARY KEY,
    contract_id INT NOT NULL,
    user_id INT NOT NULL,
    ...
    uploaded_by INT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_cronograma (contract_id, user_id),
    INDEX idx_cronograma_contrato (contract_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);
